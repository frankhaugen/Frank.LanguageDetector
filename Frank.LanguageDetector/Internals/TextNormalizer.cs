...
{
    private readonly Regex _emailRegex = new("[-_.0-9A-Za-z]{1,64}@[-_0-9A-Za-z]{1,255}[-_.0-9A-Za-z]{1,255}", RegexOptions.Compiled);
    private readonly LanguageDetectionOptions _options;
    private readonly Regex _urlRegex = new("https?://[-_.?&~;+=/#0-9A-Za-z]{1,2076}", RegexOptions.Compiled);

    public TextNormalizer(LanguageDetectionOptions options) => _options = options;
