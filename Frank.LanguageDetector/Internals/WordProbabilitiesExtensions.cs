...
{
    public static void AddLanguages(this WordProbabilities probabilities, int ngramLength = 3) => probabilities.AddLanguages(LanguageModel.Instance, ngramLength);

    public static void AddLanguages(this WordProbabilities probabilities, IEnumerable<LanguageItem> profiles, int ngramLength = 3)
    {
        foreach (var languageModel in profiles)
            probabilities.AddLanguage(languageModel, ngramLength);
    }

    internal static void AddLanguage(this WordProbabilities probabilities, LanguageItem profile, int ngramLength = 3)
    {
        foreach (var word in profile.Frequency.Keys)
        {
            if (!probabilities.ContainsKey(word)) probabilities[word] = new Dictionary<LanguageItem, double>();
